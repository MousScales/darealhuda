...
private struct LockScreenSubscriptionRequiredView: View {
    @ViewBuilder
    var body: some View {
...
struct DailyHadithArabicWidget: Widget {
    let kind: String = "DailyHadithArabicWidget"
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: DailyHadithProvider()) { entry in
            ...
            if !isSubscribed {
                ...
                if #available(iOS 17.0, *) {
                    LockScreenHadithView(data: entry.data)
                        .widgetURL(hadithURL)
                        .containerBackground(.clear, for: .widget)
                } else {
                    LockScreenHadithView(data: entry.data)
        ...
        .configurationDisplayName("Daily Hadith (Arabic)")
        .description("View today's hadith in Arabic.")
